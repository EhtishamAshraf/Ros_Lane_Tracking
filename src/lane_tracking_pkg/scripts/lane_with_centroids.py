...
"""
                        --- Saving Robot Pose while Tracking Lane using Turtlebot3 ---
This ROS node allows the Turtlebot3 Burger Robot to track the lane and save the Robot's Pose in a file.
...
2. ** Subscribes to the '/odom' topic, where it gets information about the pose of the Robot.
   The detected Pose (Position and Orientation) is stored in a file for further processing.

3. **Publishes on the `/cmd_vel` topic, where velocity commands are published. 
   Robot moves based on the velocity commands.

4. **OpenCV integration - Lane Tracking**:
...
# List to store all robot positions
position_list = []
...
def ImageCallback(image_msg):
    ...
    if contours:
        ...
        if len(largest_contours) > 0: # Second largest contour
            ...
            centroids_list.append((line_right['x'], line_right['y']))
        ...
        if len(largest_contours) > 1: # Third largest contour
            ...
            centroids_list.append((line_left['x'], line_left['y']))
...
def odom_callback(robot_pose):
    ...
    # Extract the orientation quaternion and convert it to yaw (rotation about z-axis)
    orientation_q = robot_pose.pose.pose.orientation
    orientation_list = [orientation_q.x, orientation_q.y, orientation_q.z, orientation_q.w]
    (_, _, yaw) = euler_from_quaternion(orientation_list)
    
    # Limit x, y, yaw to maximum of 2 decimal places
    x_rounded = round(pos.x, 2)
    y_rounded = round(pos.y, 2)
    yaw_rounded = round(yaw, 2)
    
    # Create a tuple of the robot's position and yaw and append to the list
    position = (x_rounded, y_rounded, yaw_rounded)
    position_list.append(position)

    # Specify the path to your .txt file and open the file in 'write mode'
    ...
    with open(file_path, 'w') as f:
        f.write(f"Robot X, Robot Y, Robot Yaw\n")
